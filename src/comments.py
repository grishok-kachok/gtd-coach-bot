...
Разговор живёт в телеграме. Комментарий — рабочая записка возле дела: пользователь
...
(«как у нас дробят задачи»), а не биография пользователя. Полная загрузка — 160 608 Б
...
class Канал:
    ...
    async def разобрать(self, заметки: list[dict], хозяин: str) -> list[dict]:
        ...
        for заметка in sorted(заметки, key=lambda n: n.get("posted_at") or ""):
            ...
            # у коуча про пользователя, работать на второго человека он не должен.
            await self._в_архив(
                "user" if свой_ли_автор else "coach",
                f"[комментарий{'' if свой_ли_автор else ', не пользователь'} "
    ...
    async def откатить(self, request_id: str | None = None) -> str:
        ...
        if not request_id:
            # Два разных ответа, и путать их нельзя: «я ничего не делал» и «всё
            # сделанное уже откачено» — это про разное, а человек по ответу
            # решает, идти ли чинить руками.
            было = await asyncio.to_thread(self.журнал.было_ли_что_откатывать)
            return ("Откатывать нечего: всё, что я делал по комментариям, уже откачено."
                    if было else
                    "Откатывать нечего: по комментариям я ничего не менял.")
...
def build_undo_server(канал: Канал):
    ...
    @tool(
        ...
        "Звать, когда пользователь говорит «откати», «верни как было», «убери то, "
