...
(решение 31.07). Так же устроена и конституция: её части лежат
в `/plugin/prompts/конституция/`, том смонтирован только на чтение, правка
текста не требует пересборки питона. Их собирает `modes.py` под режим
контекста — здесь остальные промпты, по одному файлу на прогон.
