...
def _complaints(data: dict) -> list[str]:
    ...
    словарь = data.get("словарь_голоса")
    if словарь is not None:
        if not isinstance(словарь, list):
            problems.append("«словарь_голоса» — нужен список слов через дефис")
        else:
            плохие = [w for w in словарь if not isinstance(w, str)]
            if плохие:
                problems.append(f"«словарь_голоса» — это должны быть слова, а не {плохие}")
...
- **словарь_голоса** — слова, которые распознаватель голоса регулярно калечит:
  ...
  («задача», «дедлайн», «созвон») код подставляет сам — сюда пишут только своё.
  Список короткий и точный: чужое слово в подсказке заставляет распознаватель
