...
# Строки, которые код когда-то читал, а теперь нет. Не «лишние» — на них
# не ругаемся, иначе одна забытая строка в старом файле обнуляет ВСЕ настройки
# (претензия к файлу означает откат на умолчания целиком), и человек вместо
# своей модели молча получает умолчание.
#
# `словарь_голоса` — второй дом слов для распознавателя, снят 03.08.2026.
# Свои слова человека живут в одном месте: строка `voice` в шапке глоссария,
# туда их пишет сам коуч по фразе в телеграме.
УСТАРЕВШИЕ = ("словарь_голоса",)
...
def _complaints(data: dict) -> list[str]:
    ...
    for key in УСТАРЕВШИЕ:
        if key in data:
            log.info("«%s» в настройках больше не читается — строку можно удалить", key)

    unknown = set(data) - set(DEFAULTS) - set(УСТАРЕВШИЕ)
    if unknown:
...
- **режим** — сколько коуч берёт с собой в разговор. Доступны: {modes_list}.
  `рабочий` — обычный день. `полный` — недельная, месячная и квартальная
  стратсессии, туда приезжают миссия, ценности и профиль. `годовой` — годовая
  стратсессия, там же двенадцать месяцев памяти вместо трёх. Состав каждого — файл `режимы.md`
...
**Слов для распознавателя голоса здесь нет.** Их дом — строка `voice` в шапке
`знания/глоссарий.md`, и он один: скажи коучу «это слово пишется так-то», и он
впишет его туда сам. Раньше был второй список прямо в этом файле — слово
ложилось в память, а до распознавателя не доезжало, потому что вести второй
список руками никто не помнил.
...
# Строка модели разговора в блоке YAML. Комментарий справа (если человек его
# приписал) сохраняем: он его писал, не мы.
СТРОКА_МОДЕЛИ = re.compile(r"^([ \t]*)модель_разговора[ \t]*:[^#\n]*(#[^\n]*)?$", re.MULTILINE)


def поставить_модель(brain_dir: Path, модель: str) -> bool:
    """Переписать модель разговора в файле настроек. Вернуть, получилось ли.

    Почему это делает код, а не коуч. Смена модели — механическое действие
    без единого решения: имя выбрано человеком из списка, который код же
    и показал. Просить модель переписать строку значило бы **гонять полный
    рюкзак ради правки одной строки** — и гонять его на той самой модели,
    от которой человек уходит. Фраза «переключись на Opus» при этом работать
    не перестаёт: у настройки два писателя, и это названо вслух.

    Правка точечная: одна строка на месте, остальной файл не трогаем. Файл —
    живая заметка мозга, в нём есть текст человека, и переписывать его целиком
    ради одного значения нельзя.
    """
    if модель not in KNOWN_MODELS:
        log.error("незнакомая модель %s — в настройки не пишу", модель)
        return False
    path = path_in(brain_dir)
    try:
        text = path.read_text(encoding="utf-8")
    except OSError:
        log.exception("не смог прочитать настройки %s", path)
        return False
    block = YAML_BLOCK.search(text)
    if not block:
        log.error("в настройках нет блока yaml — модель менять негде: %s", path)
        return False

    внутри = block.group(1)
    if not СТРОКА_МОДЕЛИ.search(внутри):
        log.error("в настройках нет строки «модель_разговора»: %s", path)
        return False

    def замена(m: re.Match) -> str:
        хвост = f"   {m.group(2)}" if m.group(2) else ""
        return f'{m.group(1)}модель_разговора: "{модель}"{хвост}'

    новый = СТРОКА_МОДЕЛИ.sub(замена, внутри, count=1)
    try:
        path.write_text(text[:block.start(1)] + новый + text[block.end(1):], encoding="utf-8")
    except OSError:
        log.exception("не смог записать настройки %s", path)
        return False
    log.info("модель разговора переставлена на %s", модель)
    return True
