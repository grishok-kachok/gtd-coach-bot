...
КЛАВИАТУРА = ReplyKeyboardMarkup(
    [["Что сегодня?", "Что по плану на неделю?"],
     ["Проведём недельный обзор", "Подведём месяц"],
     ["Покажи дашборд", "Переключись в полный режим"]],
    resize_keyboard=True,
)

# Во время обряда клавиатура другая: главная кнопка — выйти из него. Кнопка
# «проведём обзор» тут не нужна вовсе, а вот «закончить» должна быть под рукой
# и не требовать вспоминать формулировку.
КЛАВИАТУРА_ОБРЯДА = ReplyKeyboardMarkup(
    [["Закончить стратсессию"],
     ["Покажи дашборд", "Что по плану на неделю?"]],
    resize_keyboard=True,
)
    ...
    async def _think_and_reply(
        ...
        for номер, chunk in enumerate(куски, 1):
            ...
            await context.bot.send_message(
                chat_id=chat_id, text=chunk + хвост,
                reply_markup=КЛАВИАТУРА_ОБРЯДА if self.engine.обряд else КЛАВИАТУРА,
            )
    ...
    async def _переключить_обряд(self, text: str, context: ContextTypes.DEFAULT_TYPE) -> str:
        ...
        await context.bot.send_message(
            ...
            text=(f"{режим.значок} Включил {повод.что}. Голова другая: миссия, "
                  ...
                  f"Скажи «закончить стратсессию» — вернёмся к прежнему разговору"
                  f"{'' if отложили else ' (его пока нет — начнём с чистого листа)'}."),
            reply_markup=КЛАВИАТУРА_ОБРЯДА,
        )
