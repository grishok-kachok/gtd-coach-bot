...
КОМАНДЫ: tuple[tuple[str, str, str | None], ...] = (
    ...
    ("week", "Недельная стратсессия — план на неделю",
     "Проведём недельную стратсессию"),
    ("month", "Месячная стратсессия — план на месяц",
     "Проведём месячную стратсессию"),
    ("quarter", "Квартальная стратсессия — ревизия курса",
     "Проведём квартальную стратсессию"),
    ("year", "Годовая стратсессия — вектор на год",
     "Проведём годовую стратсессию"),
    ...
    # Свой обработчик: печатать коучу тут нечего. Ответ — механический список,
...
# Что печатает каждая команда. Собирается из того же списка, а не рядом с ним:
# два списка разъехались бы на первой же правке.
ФРАЗЫ_КОМАНД: dict[str, str] = {имя: фраза for имя, _, фраза in КОМАНДЫ if фраза}
...
class CoachBot:
    ...
    async def _think_and_reply(
        ...
        for номер, chunk in enumerate(куски, 1):
            ...
            хвост = f"\n\n{self._пометка()}" if номер == len(куски) else ""
            await context.bot.send_message(chat_id=chat_id, text=chunk + хвост)

    def _пометка(self) -> str:
        """Чем помечен ответ: значок режима, а во время обряда — ещё и словом."""
        значок = self.engine.режим().значок
        return f"{значок} стратсессия" if self.engine.обряд else значок

    async def _переключить_обряд(self, text: str, context: ContextTypes.DEFAULT_TYPE) -> str:
    ...
    async def _закончить_обряд(self, context: ContextTypes.DEFAULT_TYPE) -> str:
        ...
        if not self.engine.обряд:
            return ""
        было = self.engine.обряд
        self.engine.обряд = None
    ...
    async def on_model(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        строки = [
            f"{'▸' if код == сейчас else '·'} /{имя} — {красиво}"
            for имя, красиво, код in МОДЕЛИ
        ]
        # Незнакомое значение показываем как есть: настройки правит и человек
        # руками, и «▸» ни у кого — это не поломка, а повод увидеть, что стоит.
        известна = any(код == сейчас for _, _, код in МОДЕЛИ)
        шапка = ("Сейчас думаю моделью:" if известна
                 else f"Сейчас в настройках стоит «{сейчас}» — не из списка.")
        await update.message.reply_text(
            f"{шапка}\n\n" + "\n".join(строки) +
            "\n\nЖми нужную — разговор не прервётся, "
            "со следующей реплики буду думать ей."
        )
    ...
    def run(self) -> None:
        ...
        # Остальные команды меню — печатающая рука: одна и та же ручка на все.
        application.add_handler(CommandHandler(list(ФРАЗЫ_КОМАНД), self.on_команда))
        # Модели коуча не касаются: список и выбор — работа кода.
        application.add_handler(CommandHandler("model", self.on_model))
