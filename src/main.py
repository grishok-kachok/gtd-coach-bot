...
СТРАТСЕССИИ: tuple[tuple[str, str], ...] = (
    ("week", "Проведём недельную стратсессию"),
    ("month", "Проведём месячную стратсессию"),
    ("quarter", "Проведём квартальную стратсессию"),
    ("year", "Проведём годовую стратсессию"),
)
...
class CoachBot:
    ...
    async def on_session(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if self.engine.обряд:
            ...
            return
        строки = [f"{ПРОЧЕЕ} /{имя}" for имя, _ in СТРАТСЕССИИ]
        await update.message.reply_text(
            "Какую стратсессию проводим?\n\n" + "\n".join(строки) +
            "\n\nТекущий разговор подождёт на полке — вернёмся в него по /end."
        )
    ...
    def run(self) -> None:
        ...
        # Списки, которые печатает сам бот: коуча они не касаются.
        application.add_handler(CommandHandler("model", self.on_model))
        application.add_handler(CommandHandler("session", self.on_session))
