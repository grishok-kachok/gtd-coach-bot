...
def build_wishes_server(inbox: Inbox, todoist_token: str = "", tz: str = "Europe/Moscow"):
    @tool(
        ...
        # Схема полная, а не короткая записью `{"what": str, "why": str}`: короткая
        # объявляет ОБЯЗАТЕЛЬНЫМИ все перечисленные поля (движок проставляет
        # `required` по всем ключам). Заявка без «зачем» отбивалась валидацией
        # раньше, чем начинал работать код, — и не попадала на полку вовсе,
        # хотя код всегда читал это поле как необязательное. Поймано 13.08.2026.
        {
            "type": "object",
            "properties": {
                "what": {"type": "string",
                         "description": "Что просит уметь — своими словами пользователя"},
                "why": {"type": "string",
                        "description": "Зачем ему это, если сказал. Не сказал — не выдумывай"},
            },
            "required": ["what"],
        },
    )
