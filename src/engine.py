...
class CoachEngine:
    ...
    def _wrap(text: str, memory: str, agenda: str, strategy: str = "") -> str:
        ...
        if agenda.strip():
            blocks.append(
                "<дела_сейчас>\n"
                "Свежий агрегат из приложения FINDAY, собран кодом только что.\n"
                "FINDAY — дом дел: там их видит человек, там же они правятся.\n"
                "Это отражение — правится источник, а не отражение: кнопками\n"
                "finday_create_task, finday_close_task, finday_update_task.\n\n"
                f"{agenda}\n"
                "</дела_сейчас>"
            )
        blocks.append(text)
