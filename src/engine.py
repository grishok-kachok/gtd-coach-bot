...
class CoachEngine:
    ...
    def __init__(
        ...
        # Идёт ли обряд и с какой головой. Ставит и снимает код бота, когда
        # человек сказал «проведём обзор» или нажал кнопку. Пусто — живём
        # по настройке из мозга.
        self.обряд: str | None = None
    ...
    def _конституция(self, mode: Режим) -> str:
        ...
        # нет роли, к которой её надстраивать.
        if not общая:
            return общая
        своя = режимы_модуль.личная_надстройка(self.brain_dir)
        return f"{общая}\n\n{своя}" if своя else общая
