...
def test_вторая_находка_не_плодит_задачу_а_комментирует(todoist, tmp_path, monkeypatch):
    ...
    #
    # id находки берём из подделки, а не пишем «t1» руками: с 13.08.2026 первой
    # заводится мастер-карточка, и захардкоженный номер сдвинулся бы молча.
    находка = [з for з in todoist.tasks
               if з["content"] == backstage.FINDINGS["предложения"].title][0]
    assert todoist.comments == [
        {"task_id": находка["id"], "content": f"{backstage.МАРКЕР} вторая ночь"}]
...
# Тест «разговорное под мастер-карточку не идёт» жил здесь до 13.08.2026
# и требовал ровно обратного: стратсессии, «завал» и «предложения» обязаны
# висеть сами по себе, потому что у них другое место действия. Место действия
# осталось, а вот носитель у него сменился — теперь это пометка `разбор`
# у находки, а не то, где висит карточка (заявка #129, решение владельца).
# Что стало вместо него — test_все_находки_ложатся_подзадачами
# и test_ночные_задачи_говорят_где_разбирают.
...
@pytest.mark.parametrize("имя", backstage.ПРЕЖНИЕ_ИМЕНА)
def test_любое_прежнее_имя_подхватывается_а_не_дублируется(todoist, имя):
    """Проверяется КАЖДОЕ имя списка, а не первое. Второе появилось 13.08.2026:
    хозяин переименовал карточку у себя, и до этой правки потеря файла с id
    завела бы вторую карточку рядом с живой — дубль, который уже был в августе.
    Тест с индексом [0] такую добавку молча пропускал бы.
    """
    todoist.tasks.append({"id": "по-старому", "content": имя})
    asyncio.run(backstage.raise_task("токен", "промахи", "ночь"))
    assert backstage._запомненная_мастер_карточка() == "по-старому", имя
    assert [з for з in todoist.created if з["content"] == backstage.МАСТЕР_КАРТОЧКА] == []
...
def test_ночные_задачи_говорят_где_разбирают(todoist):
    ...
    отправляет в телеграм. С 13.08.2026 под мастер-карточкой висят все, и место
    карточки на вопрос больше не отвечает — отвечает пометка у находки."""
    for kind in ("промахи", "предложения", "завал", "недельная стратсессия"):
        todoist.tasks.append({"id": kind, "content": backstage.FINDINGS[kind].title})
    ночные = {з["kind"]: з["разбор"] for з in asyncio.run(
        backstage.ночные_задачи("токен"))}
    assert ночные == {
        "промахи": backstage.КОД,
        "предложения": backstage.РАЗГОВОР,
        "завал": backstage.РАЗГОВОР,
        "недельная стратсессия": backstage.РАЗГОВОР,
    }


def test_у_каждой_находки_пометка_из_двух_известных(todoist):
    """Опечатка в пометке молча увела бы находку из обеих кучек разбора:
    ни «код», ни «разговор» — значит разбор про неё не скажет ничего."""
    известные = {backstage.КОД, backstage.РАЗГОВОР}
    чужие = {kind: f.разбор for kind, f in backstage.FINDINGS.items()
             if f.разбор not in известные}
    assert not чужие, f"находки с непонятной пометкой: {чужие}"


def test_все_находки_ложатся_подзадачами(todoist):
    """Заявка #129: под мастер-карточку идёт ВСЁ, без исключений. Раньше
    «предложения», «завал» и четыре стратсессии висели в проекте сами по себе
    и читались как россыпь чужих дел вперемешку со своими."""
    for kind in backstage.FINDINGS:
        asyncio.run(backstage.raise_task("токен", kind, "находка"))
    мастер = [з for з in todoist.created if з["content"] == backstage.МАСТЕР_КАРТОЧКА]
    assert len(мастер) == 1, "мастер-карточка должна завестись ровно одна"
    сами_по_себе = [з["content"] for з in todoist.created
                    if з["content"] != backstage.МАСТЕР_КАРТОЧКА and not з.get("parent_id")]
    assert not сами_по_себе, f"эти находки завелись мимо мастер-карточки: {сами_по_себе}"
