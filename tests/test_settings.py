...
def test_снятая_строка_не_обнуляет_настройки(tmp_path):
    """`словарь_голоса` снят 03.08 — забытая строка не должна ронять весь файл.

    Претензия к файлу означает откат на умолчания ЦЕЛИКОМ. Если ругаться
    на устаревшую строку как на лишнюю, человек из-за одной забытой строки
    молча получит не свою модель — то есть уборка сломала бы соседнее.
    """
    write(tmp_path, 'модель_разговора: "claude-opus-5"\n'
                    'модель_ночной_работы: "claude-fable-5"\n'
    ...
    assert problems == []
    assert values["модель_разговора"] == "claude-opus-5"
    assert "словарь_голоса" not in values  # прочитан не значит принят


def test_настоящая_лишняя_строка_по_прежнему_претензия(tmp_path):
    """Снисхождение к снятой строке не должно превратиться в снисхождение ко всем."""
    write(tmp_path, 'модель_разговора: "claude-fable-5"\n'
                    'модель_ночной_работы: "claude-fable-5"\n'
                    "словарь_голоса: [Аглая]\n"
                    "любимый_цвет: синий")
    values, problems = settings.read(tmp_path)
    assert any("любимый_цвет" in p for p in problems)
    assert not any("словарь_голоса" in p for p in problems)
