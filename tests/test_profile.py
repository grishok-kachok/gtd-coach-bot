...
def test_отклик_меряет_задержку_ответа(tmp_path):
    путь = _архив(tmp_path, [
        ("2026-08-01T10:00:00+03:00", "user", "morning"),
        ("2026-08-01T10:20:00+03:00", "user", "voice"),
    ])
...
def test_пинг_без_ответа_считается_пропущенным(tmp_path):
    путь = _архив(tmp_path, [("2026-08-01T20:00:00+03:00", "user", "evening")])
    показатель = profile.отклик_на_пинги(путь, 30, date(2026, 8, 2))
...
# ── переносы: работа не считается уклонением ─────────────────────────────────
#
# Заявка владельца #59, вторая половина. Сказал «работал три часа, продолжу
# завтра» — коуч двигает срок, Todoist считает это переносом, и три дня честной
# работы делали задачу «переехавшей трижды», то есть кандидатом на «дроби или
# отпускай». Хроники это учли 06.08.2026, профиль остался считать по-старому.


def снимок(база, day, task_id, комментарии):
    """Задача в ночном снимке со своими комментариями."""
    with sqlite3.connect(база) as db:
        db.execute(
            "INSERT OR REPLACE INTO todoist_tasks(day, task_id, content, description,"
            " project, section, parent_id, labels, priority, due, deadline, duration,"
            " added_at, comments) VALUES(?,?,'дело','','Рабочее','',NULL,'',1,'','',"
            "NULL,'',?)",
            (day, task_id, json.dumps(комментарии, ensure_ascii=False)),
        )


def закрыть_с_переносами(база, task_id, переносов):
    with sqlite3.connect(база) as db:
        db.execute(
            "INSERT OR REPLACE INTO todoist_closed(task_id, completed_at, content,"
            " project, added_at, labels, priority, due, postponed)"
            " VALUES(?,'2026-08-01T09:00:00Z','дело','Рабочее','','',1,'',?)",
            (task_id, переносов),
        )


def одна_задача(база, переносов, комментарии):
    закрыть_с_переносами(база, "t1", переносов)
    снимок(база, "2026-07-31", "t1", комментарии)
    with sqlite3.connect(база) as db:
        return profile.переносы(db, 90, date(2026, 8, 2))


СЕССИЯ_1 = "2026-07-29 — 3ч, сделано: разобрал структуру, дальше: тексты"
СЕССИЯ_2 = "2026-07-30 — 2ч, сделано: тексты, дальше: проверка"
СЕССИЯ_3 = "2026-07-31 — 1,5ч, сделано: проверка, дальше: закрыть"


def test_дни_рабочих_сессий_не_считаются_переносами(база):
    """Три дня работы и три переноса — это ноль уклонений, а не хроник."""
    показатель = одна_задача(база, 3, [СЕССИЯ_1, СЕССИЯ_2, СЕССИЯ_3])
    assert "медиана 0" in показатель.значение
    assert "переехавших трижды и больше: 0" in показатель.значение


def test_без_записей_сессий_считается_как_раньше(база):
    показатель = одна_задача(база, 3, ["просто мысль вслух"])
    assert "переехавших трижды и больше: 1" in показатель.значение


def test_две_записи_за_один_день_это_один_день(база):
    """Отчитался дважды за вторник — работал один вторник и переехал один раз."""
    ещё = "2026-07-29 — 1ч, сделано: добавка, дальше: завтра"
    показатель = одна_задача(база, 2, [СЕССИЯ_1, ещё])
    assert "медиана 1" in показатель.значение


def test_сессий_больше_чем_переносов_не_уводит_в_минус(база):
    показатель = одна_задача(база, 1, [СЕССИЯ_1, СЕССИЯ_2, СЕССИЯ_3])
    assert "медиана 0" in показатель.значение and "с первого раза 100 %" in показатель.значение


def test_битый_комментарий_не_роняет_показатель(база):
    """В колонке лежит не JSON — считаем, что сессий нет, но цифру всё равно
    показываем. Молчащий показатель хуже осторожного."""
    закрыть_с_переносами(база, "t1", 4)
    with sqlite3.connect(база) as db:
        db.execute(
            "INSERT INTO todoist_tasks(day, task_id, content, description, project,"
            " section, parent_id, labels, priority, due, deadline, duration, added_at,"
            " comments) VALUES('2026-07-31','t1','дело','','Рабочее','',NULL,'',1,"
            "'','',NULL,'','это не json')"
        )
        показатель = profile.переносы(db, 90, date(2026, 8, 2))
    assert "медиана 4" in показатель.значение


def test_берётся_последний_снимок_а_не_первый(база):
    """Снимков у задачи столько, сколько дней она прожила, и комментарии в них
    копятся. Первый снимок знает про одну сессию, последний — про все три."""
    закрыть_с_переносами(база, "t1", 3)
    снимок(база, "2026-07-29", "t1", [СЕССИЯ_1])
    снимок(база, "2026-07-31", "t1", [СЕССИЯ_1, СЕССИЯ_2, СЕССИЯ_3])
    with sqlite3.connect(база) as db:
        assert "медиана 0" in profile.переносы(db, 90, date(2026, 8, 2)).значение


def test_задача_без_снимка_вовсе_не_теряется(база):
    """Закрыта в первый же день, снимка ещё не было — считаем по счётчику."""
    закрыть_с_переносами(база, "t1", 5)
    with sqlite3.connect(база) as db:
        assert "медиана 5" in profile.переносы(db, 90, date(2026, 8, 2)).значение
...
def test_показатель_на_двух_событиях_не_показывается(база, tmp_path):
    """Куплено первым живым прогоном: «приток больше оттока» было посчитано
    на двух событиях и звучало как приговор системе. Десяток событий —
    это ещё не человек, это неделя."""
    with sqlite3.connect(база) as db:
        db.execute("INSERT INTO todoist_diff VALUES('2026-08-01','появилась','t1','раз','')")
        db.execute("INSERT INTO todoist_diff VALUES('2026-08-01','закрыта','t2','два','')")
    собрано = profile.собрать(база, tmp_path / "нет-архива.db", date(2026, 8, 2))
    assert [п.имя for п in собрано] == []


def test_зрелый_показатель_проходит(база, tmp_path):
    for i in range(12):
        закрыть(база, f"t{i}", "2026-08-01T09:00:00Z")
    собрано = profile.собрать(база, tmp_path / "нет-архива.db", date(2026, 8, 2))
    assert "Часы закрытий" in [п.имя for п in собрано]
