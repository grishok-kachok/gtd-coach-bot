...
def test_свежая_часть_помечена_галочкой_и_словом():
    """Владелец посмотрел на живой ответ и сказал: непонятно, актуальная версия
    или нет. Значок читается раньше слов — он и есть ответ."""
    текст = versions.отчёт_текстом([versions.Состояние(имя="бот", стоит="5cbf6ae")])
    assert "✅" in текст and "свежий" in текст


def test_у_трёх_состояний_три_разных_значка():
    текст = versions.отчёт_текстом([
        versions.Состояние(имя="бот", стоит="aaaaaaa"),
        versions.Состояние(имя="плагин", стоит="bbbbbbb", новее="ccccccc"),
        versions.Состояние(имя="gcal-mcp", заметка="не установлен"),
    ])
    assert текст.count("✅") == 1, "свежая часть ровно одна"
    assert текст.count("🔸") == 1, "делать руками надо ровно с одной"
    assert текст.count("❓") == 1, "неизвестна ровно одна"


def test_первая_установка_без_update_sh_всё_равно_сверяется(четыре, monkeypatch):
    """Ученик собирает образ по README, sha в него не попадает. Ночь обязана
    сверять клон — иначе она молчала бы про бота ровно до того дня, когда он
    первый раз запустит ./update.sh, то есть до дня, о котором и должна сказать."""
    monkeypatch.delenv("GIT_SHA", raising=False)

    async def свежий(_клиент, коорд, _ветка):
        return "bbbbbbb" if коорд.endswith("gtd-coach-bot") else четыре[коорд]
    ...
    отчёт = asyncio.run(versions.проверить(таймаут=1))
    бот = отчёт[0]
    assert бот.заметка == "сборка неизвестна"
    assert бот.новее == "bbbbbbb", "клон отстал, а ночь этого не заметила"
